...
### 0. 간략한 회사 소개
- 회사명, 산업/핵심사업, 핵심 기술/제품을 1~2문장으로 요약하세요.
- 근거 출처를 괄호로 표기하세요. (예: 추천 회사 정보, 웹 검색 정보)
...
### 2.  준비 조언별 해설 (선택한 회사 기준으로 연결)

    - 어떤 조언 항목이 지금 질문을 해결하는 데 연결되는지
    - 반드시 선택한 회사의 채용 기준/직무 요구/문화와 어떻게 연결되는지 명시
    - 필요 시 [추천 회사 정보], [웹 검색 정보]의 문구를 간단 인용
...
intent_analysis_prompt = PromptTemplate(
    ...
    template="""당신은 대화의 맥락을 파악하여 사용자 의도를 6가지 중 하나로 분류하는 AI입니다.
대화 기록과 현재 질문을 바탕으로 가장 적절한 의도를 하나만 선택하여 소문자로 출력하세요.
(initial_search, new_search, select_job, follow_up_qa, explanation_request, chit_chat)

1. **initial_search**: 사용자가 대화의 가장 처음에서, 구체적인 조건(전공, 경력 등)을 제시하며 직무 추천을 '처음으로' 요청하는 경우.
2. **new_search**: 이미 직무 목록을 추천받은 후에, "다른 회사 추천해줘", "이거 말고 다른거" 와 같이 '새로운 목록'을 요구하는 경우.
...
5. **explanation_request**: AI가 직전에 제공한 답변(채용 공고, 조언 등)에서 사용자가 모르는 용어나 내용을 질문하는 경우. (예: "여기서 말한 MLOps가 뭐에요?", "그 '프롬프트 엔지니어링'은 무슨 뜻이에요?")
6. **chit_chat**: "고마워", "안녕" 등 직무 추천과 직접적인 관련이 없는 일상적인 대화.
...
explain_term_prompt = PromptTemplate(
    input_variables=["last_answer", "question"],
    template="""
당신은 사용자가 이전에 받은 답변을 더 잘 이해하도록 돕는 친절한 AI 어시스턴트입니다.
...
[지시사항]
1. 질문에서 사용자가 궁금해하는 용어나 내용을 정확히 찾아내세요.
2. 해당 개념을 초보자도 이해할 수 있도록, 간단한 정의 + 관련 예시를 제시하세요.
3. 직무나 채용과 연결될 경우, 그 맥락도 함께 설명하세요.
